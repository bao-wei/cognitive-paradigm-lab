# 学习目标

理解变化检测和变化定位范式如何测量视觉工作记忆，并能够解释正确率、反应时及条件差异。

# 实验原理

视觉工作记忆容量有限。被试短暂记忆一组彩色方块，随后判断指定位置的颜色是否改变，或指出发生变化的位置。表现越准确，通常说明对视觉信息的保持与比较越稳定。

# 任务流程

1. 观察并记忆屏幕上的 6 个彩色方块。
2. 在变化检测阶段，判断指定位置的颜色与记忆画面相同还是不同；电脑按 Y 或 N，手机点击“Y / 是”或“N / 否”。
3. 在变化定位阶段，比较前后两组方块；电脑按 1–6，手机点击对应数字指出颜色发生变化的位置。
4. 程序记录每次反应的正确性、按键和反应时。

# 核心指标

- **变化检测正确率**：字段 `key_resp.corr`，数值越高表示判断越准确。
- **变化检测反应时**：字段 `key_resp.rt`，在保证准确的前提下越短表示反应越快。
- **变化定位正确率**：字段 `localisation_resp.corr`，反映识别变化位置的能力。
- **实验条件**：字段 `condition_label`，用于比较相同与变化试次。

# 结果解读

应同时查看正确率和反应时，避免把单纯的快速反应解释为更好的表现。相同与变化条件之间的差异可反映任务难度或判断偏向；变化定位正确率可作为视觉工作记忆表现的补充指标。

# 注意事项

请在安静环境中使用键盘或屏幕下方的触控按钮完成任务，保持注视屏幕中央，并在发布前完整试做一次，确认材料、输入和结果回传均正常。

# 来源与参考

Zhao, C., Vogel, E. & Awh, E. Change localization: A highly reliable and sensitive measure of capacity in visual working memory. Atten Percept Psychophys 85, 1681–1694 (2023). https://doi.org/10.3758/s13414-022-02586-0
