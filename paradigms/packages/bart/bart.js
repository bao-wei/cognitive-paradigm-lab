...
let expInfo = {
    'participant': 'teaching-session',
};
const cognitionLabTrials = [];
...
flowScheduler.add(psychoJS.serverManager.waitForResources());
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    trialcount.setText(((("气球：" + (trials.thisN + 1).toString()) + "/") + trials.nTotal.toString()));
    // keep track of which components have finished
    trialComponents = [];
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    if (popped || _pj.in_es6("return", bankButton.keys)) {
      cognitionLabTrials.push({
        trial: trials.thisN + 1,
        nPumps: pump.thisN + 1,
        size: balloonSize,
        earnings: thisBalloonEarnings,
        popped,
      });
    }
...
function feedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    if ((popped === true)) {
        feedbackText = "气球爆炸，本轮收益归零";
        pop_sound.play();
    } else {
        feedbackText = ("本轮已保存 \u00a3" + util.round(lastBalloonEarnings, 2).toString());
    }
    
    feedbacktxt.setText(feedbackText);
    trialcount_2.setText(((("气球：" + (trials.thisN + 1).toString()) + "/") + trials.nTotal.toString()));
    // keep track of which components have finished
    feedbackComponents = [];
